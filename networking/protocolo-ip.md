...
Por esses motivos, um modelo híbrido de 5 camadas foi proposto por Tanenbaum para melhorar as deficiências do modelo TCP/IP e retirar os excessos do modelo OSI.
...
> Um pacote, nada mais é que um pedaço pequeno de uma mensagem maior enviado pela rede. 
...
Atualmente, os serviços de rede não criam canais dedicados para realizar a transmissão de dados, tenho que dizer que essa é uma meia-verdade mas esse é um assunto para tratarmos em um artigo exclusivo. Em resumo, um pacote, é um fragmento de uma mensagem e que são transmitidos individualmente, sem uma rota pré-definida até o destino e possuem a vantagem de não ocupar o canal de comunicação em períodos de ociosidade. Caso alguma rota falhe, os pacotes podem tomar caminhos alternativos, minimizando os períodos de interrupções. Como não há circuitos *dedicados*, a rede consegue lidar com um volume muito maior de operações simultâneas.
...
> Datagrama: unidade de transferência básica que fornece serviço de comunicação não-confiável em redes comutadas por pacotes.
